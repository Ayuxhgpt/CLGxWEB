...
async function sendEmailWithRetry(payload: CreateEmailOptions, retries = 1, delay = 500) {
    for (let i = 0; i <= retries; i++) {
        try {
            ...
            if (data.error) {
                throw new Error(data.error.message); // Explicitly throw if Resend returns an error object
            }
...
export async function sendVerificationEmail(email: string, otp: string) {
    ...
    // Use retry wrapper
    // If it fails after retries, it triggers the "catch" block in the caller
    return await sendEmailWithRetry(emailPayload);
}
