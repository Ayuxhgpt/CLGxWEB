...
import {
    Search, Shield, ShieldOff, Ban, CheckCircle,
...
export default function UserManagementPage() {
    ...
    // Helper to render modal content cleanly
    const ModalContent = () => {
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-[var(--text-primary)]">{content.title}</h3>
                        <p className="text-sm text-[var(--text-secondary)]">{content.desc}</p>
                    </div>
                </div>

                <div className="bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-subtle)]">
            ...
            <main className="container mx-auto px-4 pt-24 pb-12">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    <Card className="mb-6 p-4 border-[var(--border-subtle)]">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <select
                                className="bg-[var(--bg-surface)] border border-[var(--text)/0.1] rounded-lg px-3 py-2 text-[var(--text-primary)] outline-none focus:ring-2 focus:ring-[var(--primary)] transition-all"
                                value={roleFilter}
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    <Card className="overflow-hidden p-0 border-[var(--border-subtle)]">
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-[var(--border-subtle)]">
                                    {loading ? (
                                        ...
                                        users.map((user) => (
                                            <tr key={user._id} className="hover:bg-[var(--bg-surface-2)]/50 transition-colors group">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center">
                                                        <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 p-[1px] mr-3 shadow-sm">
                                                            <div className="h-full w-full rounded-full bg-[var(--bg-main)] flex items-center justify-center text-xs font-bold text-white">
                                                ...
                                                <td className="px-6 py-4">
                                                    {user.isBlocked ? (
                                                        <span className="flex items-center text-red-400 text-xs font-medium bg-red-500/10 w-fit px-2 py-1 rounded-full border border-red-500/20">
                                                            <Ban className="h-3 w-3 mr-1" /> Blocked
                                                        </span>
                                                    ) : (
                                                        <span className="flex items-center text-green-400 text-xs font-medium bg-green-500/10 w-fit px-2 py-1 rounded-full border border-green-500/20">
                        ...
                        {/* Pagination */}
                        <div className="px-6 py-4 border-t border-[var(--border-subtle)] flex justify-between items-center bg-[var(--surface)]/30">
...
// Helper for Link wrapped button
function LinkHref({ href, children }: { href: string, children: React.ReactNode }) {
    const router = useRouter();
    return <div onClick={() => router.push(href)} className="cursor-pointer">{children}</div>
}
